...
# by Anirudh Goyal, Alex Lamb, Jordan Hoffmann, Shagun Sodhani, Sergey Levine, Yoshua Bengio, Bernhard Schölkopf
# https://iclr.cc/virtual/2021/poster/3224
# https://github.com/dido1998/Recurrent-Independent-Mechanisms/blob/master/RIM.py
...
class RIMs(BaseLitModule):
	def __init__(self, d_hidden, n_units, n_active_RIMs, n_layers=1, rnn_cell="LSTM", bidirectional=False, autoregressive=False, loss="MSE", **kwargs):
		super().__init__(**kwargs)
		self.model_architecture = "RIMs"

		assert n_active_RIMs <= n_units, "Number of active RIMs must be less than or equal to number of units"
		assert rnn_cell in ["LSTM", "GRU"], "RNN type must be 'LSTM' or 'GRU'"
		...
		rim_dim = d_hidden * n_units * (1 + int(bidirectional))
		self.autoregressive = autoregressive
		if not self.autoregressive:
			self.fc = nn.Linear(rim_dim, self.d_features * self.d_seq_out)
		else:
			self.fc = nn.Linear(rim_dim, self.d_features)

		self.loss_fn = self.metrics[loss].clone()

	def _extract_top_layer(self, h, b_size):
		h = h.view(self.n_layers, 1 + int(self.bidirectional), b_size, self.d_hidden * self.n_units)
		h_top = h[-1]  # shape: (num_directions, batch, hidden_size)
		if self.bidirectional:
			# forward is h_top[0], backward is h_top[1]
			return torch.cat([h_top[0], h_top[1]], dim=1)  # (batch, 2*hidden_size)
		else:
			return h_top[0]  # (batch, hidden_size)
	...
	def _shared_step(self, x, y):
		...
		if not self.autoregressive:
			h_top = self._extract_top_layer(h, b_size)
			y_pred = self.fc(h_top).view(b_size, self.d_seq_out, self.d_features)
		else:
			preds = []
			h_top = self._extract_top_layer(h, b_size)
			out_step = self.fc(h_top).unsqueeze(1)
			preds.append(out_step)
			for _ in range(self.d_seq_out - 1):
				out_step_t = out_step.transpose(0, 1)
				if self.rnn_cell == "LSTM":
					_, h, c = self.RIM(out_step_t, h, c)
				else:
					_, h = self.RIM(out_step_t, h)
				h_top = self._extract_top_layer(h, b_size)
				out_step = self.fc(h_top).unsqueeze(1)
				preds.append(out_step)
...
class GroupGRUCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        for w in self.parameters():
            w.data.uniform_(-std, std)  # initialize uniformly instead of ones
...
class RIMCell(nn.Module):
	def __init__(self, 
		input_size, hidden_size, num_units, k, rnn_cell, input_key_size = 64, input_value_size = 400, input_query_size = 64,
		...
		self.key = nn.Linear(input_size, num_input_heads * input_query_size)
	...
	def input_attention_mask(self, x, h):
		...
		mask_ = torch.zeros(x.size(0), self.num_units).to(x.device)
	...
	def forward(self, x, hs, cs = None):
		...
		"""
		size = x.size()
		null_input = torch.zeros(size[0], 1, size[2]).float().to(x.device)
...
class RIMModule(nn.Module):
	def __init__(self, input_size, hidden_size, num_units, k, rnn_cell, n_layers, bidirectional, **kwargs):
		super().__init__()
		self.n_layers = n_layers
		...
		if self.num_directions == 2:
			self.rimcell = nn.ModuleList([RIMCell(input_size, hidden_size, num_units, k, rnn_cell, **kwargs) if i < 2 else 
				RIMCell(2 * hidden_size * self.num_units, hidden_size, num_units, k, rnn_cell, **kwargs) for i in range(self.n_layers * self.num_directions)])
		else:
			self.rimcell = nn.ModuleList([RIMCell(input_size, hidden_size, num_units, k, rnn_cell, **kwargs) if i == 0 else
			RIMCell(hidden_size * self.num_units, hidden_size, num_units, k, rnn_cell, **kwargs) for i in range(self.n_layers)])
	...
	def forward(self, x, h = None, c = None):
		...
		hs = torch.split(h, 1, 0) if h is not None else torch.split(torch.randn(self.n_layers * self.num_directions, x.size(1), self.hidden_size * self.num_units).to(x.device), 1, 0)
		hs = list(hs)
		cs = None
		if self.rnn_cell == 'LSTM':
			cs = torch.split(c, 1, 0) if c is not None else torch.split(torch.randn(self.n_layers * self.num_directions, x.size(1), self.hidden_size * self.num_units).to(x.device), 1, 0)
			cs = list(cs)
